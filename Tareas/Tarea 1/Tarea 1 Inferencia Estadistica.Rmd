...
ggplot(df, aes(x = Peso)) +
  geom_histogram(binwidth = 1, fill = "#5D478B", color = "#8968CD") +
...
# Histograma 
ggplot(df, aes(x = Peso)) +
  geom_histogram(binwidth = 2, fill = "#5D478B", color = "#8968CD") +
  labs(title = "Histograma con intervalos de clase 1", 
       x = "Peso soportado", 
       y = "Frecuencia") +
  theme_minimal() +
...
```{r}
# Diagrama de caja y brazos con el percentil 75 indicado
ggplot(df, aes(x = "", y = Peso)) +
  geom_boxplot(fill = "lightcoral") +
  geom_hline(yintercept = peso_75, color = "blue", linetype = "dashed", size = 1) +
...
ggplot(datos, aes(x = "", fill = tipo)) +
  ...
  # Añadir borde blanco entre las secciones
  coord_polar(theta = "y") +               
  # Convertir el gráfico de barras en gráfico de pastel
  labs(title = "Distribución de Grupos Sanguíneos",
       fill = "Tipo de Sangre") +          
  # Título y etiqueta de la leyenda
  theme_void() + 
  # Quitar etiquetas de ejes y fondo
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"), 
        # Centrar título y ajustar tamaño y fuente
        legend.position = "right") +      # Ubicar la leyenda a la derecha
  scale_fill_manual(values = c("skyblue", "lightgreen", "lightcoral", "lightgoldenrod", "lightpink", "lightcyan", "lightyellow", "lightgray")) 
```
