...
datos <- read.csv("climaOrganizacional.txt", sep=",")
```
...
tabla_sexos <- table(datos$Sexo)

# Calcular el porcentaje
porcentaje_sexos <- prop.table(tabla_sexos) * 100
print(porcentaje_sexos)
```
...
tabla_edad <- table(datos$Rango.de.edad)
print(tabla_edad)

# Si necesitas estadísticas descriptivas adicionales
library(dplyr)
# Extraer el rango mínimo de edad de las categorías y convertir a numérico
datos$Edad <- as.numeric(gsub("^[^0-9]*", "", datos$Rango.de.edad))

# Verificar y mostrar los valores NA para entender por qué se introdujeron
valores_na <- datos$Rango.de.edad[is.na(datos$Edad)]
print(valores_na)

# Imprimir resumen de la edad
estadisticas_edad <- summary(datos$Edad)
